...
function checkLifeCycle(){
 	for (m=0; m < model.data.machines[model.selectedMachine].components.length; m++){ // loop thru all items
		Select Case m=0; m < model.data.machines[model.selectedMachine].components.length; m++

		Case 1 parseFloat(model.data.machines[model.selectedMachine].components[m].hours) >= parseFloat(model.data.machines[model.selectedMachine].components[m].projection){
		...
		Case 2 parseFloat(model.data.machines[model.selectedMachine].components[m].hours) > parseFloat(model.data.machines[model.selectedMachine].components[m].projection) * 0.89) && < parseFloat(model.data.machines[model.selectedMachine].components[m].projection)*.99{
